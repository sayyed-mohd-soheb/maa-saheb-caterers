...
function showSection(id, btn) {
    // 1. Purane active sections aur buttons ko inactive karo
    document.querySelectorAll('.section').forEach(s => s.classList.remove('active'));
    document.querySelectorAll('.nav-item').forEach(b => b.classList.remove('active'));
    
    // 2. Naye wale ko active karo
    document.getElementById(id).classList.add('active');
    btn.classList.add('active');
    
    // 3. FAB (Plus button) sirf Home par dikhe
    const fabBtn = document.getElementById('fabBtn');
    if (fabBtn) {
        fabBtn.style.display = id === 'home' ? 'flex' : 'none';
    }

    // 4. 🚨 NAYA LOGIC: Green Snackbar ko sirf Menu tab par dikhao
    const snackbar = document.getElementById('cartSnackbar');
    if (snackbar) {
        if (id === 'menu') {
            // Agar Menu par aaye, toh calculation run karo (agar items hain toh khud pop up hoga)
            updateCartSnackbar();
        } else {
            // Kisi aur page par gaye, toh zabardasti chupao
            snackbar.classList.remove('show');
        }
    }

    // 5. Page ko top par scroll kardo
...
/* ==========================================================================
   UPI INTENT PAYMENT — SIMPLE DIRECT FLOW
   Place Order → UPI app opens instantly → PIN → Back → Success popup → Home
   UPI: 9833759589@ptaxis (Maa-Saheb Caterers)
   ========================================================================== */

function placeOrder() {
    // Get dynamic cart total
    const totalText = document.getElementById('summaryTotalAmount').innerText;
    ...
    // Fire UPI intent — user selects their UPI app and pays
    window.location.href = upiDeepLink;

    // When user comes back from UPI app, show success popup
    // (visibilitychange fires when browser tab regains focus after UPI app closes)
...
function showUpiSuccess() {
    const popup = document.getElementById('upiSuccessPopup');
    popup.style.display = 'flex';
}

function upiSuccessDone() {
    // Hide popup
    document.getElementById('upiSuccessPopup').style.display = 'none';
    // Clear cart
    cart = {};
    updateCartSnackbar();
